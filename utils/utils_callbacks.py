    ...
    def ver_test(self, backbone: torch.nn.Module, global_step: int):
        ...
        for i in range(len(self.ver_list)):
            acc1, std1, acc2, std2, xnorm, embeddings_list = verification.test(
                self.ver_list[i], backbone, 512, 10)
            ...
            if self.init:
                ...
                if fp16:
                    msg = "Speed %.2f samples/sec   Loss %.4f   LearningRate %.6f   Epoch: %d   Global Step: %d   " \
                          "Fp16 Grad Scale: %2.f   Required: %1.f hours Feat Norm %.4f  Reg Loss %.4f" % (
                              speed_total, loss.avg, learning_rate, epoch, global_step,
                              grad_scaler.get_scale(), time_for_end, feat_norm, reg_loss
                          )
                else:
                    msg = "Speed %.2f samples/sec   Loss %.4f   LearningRate %.6f   Epoch: %d   Global Step: %d   " \
                          "Required: %1.f hours  Feat Norm %.4f  Reg Loss %.4f" % (
                              speed_total, loss.avg, learning_rate, epoch, global_step, 
                              time_for_end, feat_norm, reg_loss
                          )
                logging.info(msg)
                loss.reset()
                self.tic = time.time()
